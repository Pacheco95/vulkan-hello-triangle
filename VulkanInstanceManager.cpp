...
namespace engine {
    ...
    void VulkanInstanceManager::createInstance() {
        ...
        std::vector<const char *> requiredExtensions = WindowManager::getRequiredExtensions();
        ...
        if (ApplicationConfig::IS_VALIDATION_LAYERS_ENABLED) {
            vkInstanceCreateInfo.enabledLayerCount = static_cast<uint32_t>(ApplicationConfig::VALIDATION_LAYERS.size());
            vkInstanceCreateInfo.ppEnabledLayerNames = ApplicationConfig::VALIDATION_LAYERS.data();
